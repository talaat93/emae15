...
.fc .fc-toolbar-title { font-size: 1.1rem; font-weight: 700; color: var(--d-t1); }
.fc .fc-button { background: var(--d-card-2); border: 1px solid var(--d-border); color: var(--d-t1); font-size: .78rem; font-weight: 600; border-radius: 6px; }
.fc .fc-button:hover { background: var(--d-card-2); border-color: var(--d-border-2); color: var(--d-t1); }
.fc .fc-button-primary:not(:disabled).fc-button-active,
.fc .fc-button-primary:not(:disabled):active { background: var(--d-navy); border-color: var(--d-navy); color: #fff; }
.fc .fc-col-header-cell-cushion { color: var(--d-t2); font-size: .75rem; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; }
.fc .fc-daygrid-day-number, .fc .fc-daygrid-day-top a { color: var(--d-t2); font-size: .8rem; }
.fc .fc-daygrid-day.fc-day-today, .fc .fc-timegrid-col.fc-day-today { background: rgba(238,125,26,.06) !important; }
.fc-theme-standard td, .fc-theme-standard th, .fc-theme-standard .fc-scrollgrid { border-color: var(--d-border); }
.fc .fc-timegrid-slot-label { color: var(--d-t2); font-size: .72rem; }
...
.fc-business-hours { background: var(--d-card-2); }
.legend-dot { display: inline-block; width: 10px; height: 10px; border-radius: 50%; margin-right: .35rem; }
.today-item { display: flex; gap: .65rem; align-items: flex-start; padding: .7rem .85rem; border-bottom: 1px solid var(--d-border); cursor: pointer; transition: background .15s; }
.today-item:hover { background: var(--d-card-2); }
.today-item:last-child { border-bottom: none; }
.today-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; margin-top: .35rem; }
.today-title { font-size: .8rem; font-weight: 600; color: var(--d-t1); line-height: 1.35; }
.today-meta { font-size: .72rem; color: var(--d-t2); margin-top: .15rem; }
</style>';
...
<div class="d-content" style="padding:1rem;">
  <div style="display:grid;grid-template-columns:1fr 280px;gap:1rem;height:calc(100vh - 120px);">
    ...
    <div style="background:var(--d-card-2);border-radius:12px;padding:1rem;border:1px solid var(--d-border);display:flex;flex-direction:column;overflow:hidden;">
      <div id="calendar" style="flex:1;min-height:0;"></div>
      <!-- Légende catégories -->
      <div style="display:flex;flex-wrap:wrap;gap:.5rem 1rem;padding:.75rem 0 .25rem;border-top:1px solid var(--d-border);margin-top:.5rem;">
        <?php foreach ($categoryConfig as $key => $cat): ?>
        <span style="display:inline-flex;align-items:center;font-size:.72rem;color:var(--d-t2);">
    ...
    <!-- Sidebar aujourd'hui -->
    <div style="background:var(--d-card-2);border-radius:12px;border:1px solid var(--d-border);display:flex;flex-direction:column;overflow:hidden;">
      <div style="padding:.85rem 1rem;border-bottom:1px solid var(--d-border);flex-shrink:0;">
        <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--d-t2);">Aujourd'hui</div>
        <?php
      ...
      <div style="padding:.5rem .5rem .35rem;border-bottom:1px solid var(--d-border);flex-shrink:0;">
        <div style="font-size:.78rem;font-weight:700;color:var(--d-t1);padding:.35rem .4rem;"><?= $days[$dow] ?> <?= $d ?> <?= $months[$mo] ?> <?= $yr ?></div>
        <div style="font-size:.72rem;color:var(--d-t2);padding:0 .4rem .35rem;"><?= count($todayInterventions) ?> intervention<?= count($todayInterventions) > 1 ? 's' : '' ?> planifiée<?= count($todayInterventions) > 1 ? 's' : '' ?></div>
      </div>
      <div style="flex:1;overflow-y:auto;">
        <?php if (empty($todayInterventions)): ?>
          <div style="padding:2rem 1rem;text-align:center;color:var(--d-t2);font-size:.82rem;">
          ...
          <a href="<?= e(url_for('dispatcher/intervention_view.php?id='.$interv['id'])) ?>" style="display:block;text-decoration:none;">
            <div class="today-item">
              ...
              <div style="flex:1;min-width:0;">
                <div class="today-title"><?= e($clientName) ?></div>
                <?php if ($time): ?>
                <div class="today-meta"><?= e($time) ?><?= !empty($interv['client_city']) ? ' · '.e($interv['client_city']) : '' ?></div>
                <?php endif; ?>
      ...
      <div style="padding:.75rem;border-top:1px solid var(--d-border);flex-shrink:0;">
        <a href="<?= e(url_for('dispatcher/interventions.php')) ?>" style="display:block;text-align:center;font-size:.75rem;color:var(--d-t2);text-decoration:none;padding:.4rem;border-radius:6px;background:var(--d-card-2);border:1px solid var(--d-border);">Toutes les interventions →</a>
      </div>
    </div>
